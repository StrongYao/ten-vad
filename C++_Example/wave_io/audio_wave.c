...
static int wav_init(SWavFile* self, const char* filename, const char* mode) {
  ...
  if (!self->m_fp)
    ...
    if (strcmp(riff, "RIFF") == 0 && strcmp(wavefmt, "WAVEfmt") == 0) {
      ...
      if (fread(&self->m_head.FormatCategory, sizeof(int16_t), 1, self->m_fp) == 0) {
        return -1;
      }
      if (fread(&self->m_head.NChannels, sizeof(int16_t), 1, self->m_fp) == 0) {
        return -1;
      }
      if (fread(&self->m_head.SampleRate, sizeof(int32_t), 1, self->m_fp) == 0) {
        return -1;
      }
      if (fread(&self->m_head.byteRate, sizeof(int32_t), 1, self->m_fp) == 0) {
        return -1;
      }
      if (fread(&self->m_head.BytesPerSample, sizeof(int16_t), 1, self->m_fp) == 0) {
        return -1;
      }
      if (fread(&self->m_head.NBitsPerSample, sizeof(int16_t), 1, self->m_fp) == 0) {
        return -1;
      }
      ...
      if (fread(&self->m_head.RawDataFileLength, sizeof(int32_t), 1, self->m_fp) == 0) {
        return -1;
      }
      scan2data(self);
      // save DATA position
    ...
    self->m_head.BytesPerSample = self->m_head.NChannels * self->m_head.NBitsPerSample / 8;
    self->m_head.byteRate = self->m_head.BytesPerSample * self->m_head.SampleRate;
    rewind(self->m_fp);
...
size_t wav_read_interleave(SWavFile* self, void* buffer, size_t read_bytes) {
  ...
  for (int i = 0; i < channel_num; i++) {
    for (int j = 0; j < frame_size; j++) {
      uint8_t* dst = (uint8_t*)buffer + (j * channel_num + i) * bytes_per_sample;
      uint8_t* src = temp + (j * channel_num + i) * bytes_per_sample;
      memcpy(dst, src, bytes_per_sample);
    }
  }
...
size_t wav_write_sequential(SWavFile* self, const void* buffer, size_t write_bytes) {
  ...
  for (int sampleCnt = 0; sampleCnt < frame_size; sampleCnt++) {
    for (int channelCnt = 0; channelCnt < channel_num; channelCnt++) {
      uint8_t* dst = temp + (sampleCnt * channel_num + channelCnt) * bytes_per_sample;
      uint8_t* src =
          (uint8_t*)buffer + (channelCnt * frame_size + sampleCnt) * bytes_per_sample;
      memcpy(dst, src, bytes_per_sample);
    }
  }
...
size_t wav_write_interleave(SWavFile* self, const void* buffer, size_t write_bytes) {
  uint8_t* temp = (uint8_t*)malloc(write_bytes);
  int bytes_per_sample = self->m_head.NBitsPerSample / 8;
  int channel_num = self->m_head.NChannels;
  int frame_size = (int)(write_bytes / (bytes_per_sample * channel_num));
  for (int sampleCnt = 0; sampleCnt < frame_size; sampleCnt++) {
    for (int channelCnt = 0; channelCnt < channel_num; channelCnt++) {
      ...
      uint8_t* src =
          (uint8_t*)buffer + (sampleCnt * channel_num + channelCnt) * bytes_per_sample;
      memcpy(dst, src, bytes_per_sample);
    }
  }
...
size_t wav_write_interleave_stereo(SWavFile* self, const void* buffer1, const void* buffer2,
                                   size_t write_bytes) {
...
uint16_t wav_get_num_channels(const SWavFile* self) { return self->m_head.NChannels; }

uint32_t wav_get_sample_rate(const SWavFile* self) { return self->m_head.SampleRate; }

size_t wav_get_sample_size(const SWavFile* self) { return self->m_head.NBitsPerSample; }

size_t wav_get_length(const SWavFile* self) { return self->m_head.FileLength; }

int32_t wav_get_data_length(const SWavFile* self) { return self->m_head.RawDataFileLength; }
